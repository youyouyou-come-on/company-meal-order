...
- 📋 菜单展示（本周/下周可预览，点餐只开放今天和下一个可点餐日）
- ✅ 按份数点餐或取消
- 👥 实时显示已报名人员和人数
- ⏰ 自动截止（午餐 10:00 前，晚餐 15:30 前）
- 🍽️ 实名建议专区（登录员工提交并显示真实姓名）
- 🔧 管理后台（密码保护，菜单增删改、复制上周菜单、Excel/CSV 导入导出）
...
| `DINGTALK_REMINDER_FIRST_CRON` | 上午首次提醒时间 | `30 9 * * 1-6` |
| `DINGTALK_REMINDER_SECOND_CRON` | 上午二次提醒时间 | `50 9 * * 1-6` |
| `DINGTALK_DINNER_REMINDER_FIRST_CRON` | 晚餐首次提醒时间 | `0 15 * * 1-6` |
| `DINGTALK_DINNER_REMINDER_SECOND_CRON` | 晚餐二次提醒时间 | `20 15 * * 1-6` |
...
登录后进入 **管理菜单** 并通过管理员验证，可以把当前所选周导出为以日期区间命名的 `.xlsx` 文件，例如 `2026.08.10-2026.08.15.xlsx`。工作表采用横向一周菜单版式：顶部显示周区间，左侧显示日期和星期，中间分别展示午餐和晚餐菜品，并预设为 A4 横向单页打印。

推荐先导出模板，再用 Excel 填写后直接导入。系统可以识别横向周菜单 `.xlsx`，也兼容原来的三列表格 `.xlsx` 和 CSV；三列表格表头必须包含 `日期`、`餐次`、`菜品`，日期格式为 `YYYY-MM-DD`，餐次支持 `午餐`、`晚餐`、`lunch`、`dinner`。同一份文件可以同时包含本周、下周和后续周的日期；导入会新增或覆盖同日期同餐次的菜单，空白菜品行会被忽略，不会删除已有菜单。整份文件会先校验，任意一行错误时不会写入任何菜单。

## 💬 实名建议

建议专区仅限已登录员工查看和提交。已有建议和新建议都会显示提交员工的真实姓名，未登录访问不会返回建议内容或员工姓名。
...
提醒建议配置为午餐 `09:30`、`09:50` 和晚餐 `15:00`、`15:20`。脚本会先统计当天对应餐次已经提交点餐的员工人数，只要已有 1 人提交，就会启动未点餐提醒；如果当天对应餐次还没有任何人提交，会认为今天可能是休假日，直接跳过提醒，避免误打扰全员。这个 1 人判断不区分员工是否绑定钉钉 UserId；真正发送时只会发给已绑定 UserId 的未点餐员工。
...
pnpm exec tsx scripts/send-dingtalk-meal-reminders.ts --meal=lunch --round=first
...
DINGTALK_AGENT_ID="你的 AgentId" \
DINGTALK_REMINDER_URL="https://meal.zcgc.club" \
pnpm exec tsx scripts/send-dingtalk-meal-reminders.ts --meal=dinner --round=first
```
...
DINGTALK_AGENT_ID="你的 AgentId" \
DINGTALK_REMINDER_URL="https://meal.zcgc.club" \
pnpm exec tsx scripts/send-dingtalk-meal-reminders.ts --meal=lunch --round=first --live
...
DINGTALK_AGENT_ID="你的 AgentId" \
DINGTALK_REMINDER_URL="https://meal.zcgc.club" \
pnpm exec tsx scripts/send-dingtalk-meal-reminders.ts --meal=dinner --round=first --live
```
