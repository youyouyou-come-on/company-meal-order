...
- 🔧 管理后台（密码保护，菜单增删改、复制上周菜单、CSV 导入导出）
...
登录后进入 **管理菜单** 并通过管理员验证，可以导出当前所选周的 CSV 菜单。导出的文件带有 Excel 可识别的 UTF-8 标记，包含周一至周六的午餐和晚餐共 12 行。

推荐先导出模板，再用 Excel 填写后导入。表头必须包含 `日期`、`餐次`、`菜品`：日期格式为 `YYYY-MM-DD`，餐次支持 `午餐`、`晚餐`、`lunch`、`dinner`。同一份 CSV 可以同时包含本周、下周和后续周的日期；导入会新增或覆盖同日期同餐次的菜单，空白菜品行会被忽略，不会删除已有菜单。整份文件会先校验，任意一行错误时不会写入任何菜单。
